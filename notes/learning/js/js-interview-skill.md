...
DTD (document type definition, 文档类型定义)：告诉浏览器此文档的类型，浏览器根据它绝对用哪种协议来解析
...
定义：当各种盒子的位置、大小及其他属性，例如颜色、字体大小等都确定下来后，浏览器于是便把这些元素都按照各自的特性绘制了一遍，于是页面的内容出现了，这个过程就称之为repaint。

大白话：页面需要呈现的内容只要有一丁点改变都需要进行重绘repaint，例如改变元素背景色。

触发Repaint：DOM改动和CSS改动

优化重排和重绘：

* 尽量一次性修改，即不要逐个改变样式。例如一次性加个class
* ”离线“的批量改变和变现DOM。
...
* 学习能力：不断学习能的业务和技术，沉淀、总结
* 无上限的付出：对于无法解决的问题可以熬夜、加班

### 职业规划

* 目标是什么：在业务上称为专家，在技术上称为行业大牛
* 近阶段的目标：不断的学习积累各方面的经验，以学习为主
* 长期目标：做几件很有价值的事情，如开源作品、技术框架等
* 方式方法：先完成业务的主要问题，做到极致，然后逐步向目标靠拢
