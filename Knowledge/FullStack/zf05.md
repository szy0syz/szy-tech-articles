    ...
    if (siblingsUlList.length > 0 && (tagName === 'em' || tagName === 'span')) {
      ...
      for (var i = 0, len = siblingsUlList.length; i < len; i++) {
        ...
        if (isHide) {
          ...
          var nodes = siblingsUlList[i].childNodes;
          for (var j = 0, len1 = nodes.length; j < len1; j++) {
            // 我靠，text没有tagName属性！！！靠！！
            // 业务逻辑：过滤出元素节点且是ul标签的才进行隐藏，其它元素没必要嘛
            if (nodes[j].nodeType === 1 && nodes[j].tagName.toLowerCase() === 'ul') {
