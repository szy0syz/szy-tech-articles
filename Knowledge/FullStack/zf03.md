...
- 大概步骤：
  - Ajax远程获取后端数据 -> json格式
  - 数据绑定 -> 把json数据转换对象后绑定到页面元素上，三种方法中选字符串拼接
  - 图片延迟加载
  - 页面加载完毕500ms后加载首屏图片
...
## 课时65：手写getElementsByClassName的实现

- 实现原理简述：
  - 将传入的类名字符串先去两遍空格，再split分割成数组 
  - 使用`.getElementsByTagName('*')`获取该元素下的所有标签
  - 循环每个标签拿到标签的className是否包含传入的类名
  - 在上面那个循环里再循环分割出来的类名数组，假设法验证，通过就push
  - 返回结果
